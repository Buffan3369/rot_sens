...
message(paste0(round(length(which(cenozoic$lat_sd > 5))/nrow(cenozoic), digits = 3)*100, 
               "% of cells have a latitudinal standard deviation value more than 5°"))
mesozoic <- df[which(df$time > 66 & df$time <= 252), ]
message(paste0(round(length(which(mesozoic$lat_sd > 5))/nrow(mesozoic), digits = 3)*100, 
               "% of cells have a latitudinal standard deviation value more than 5°"))
palaeozoic <- df[which(df$time > 252), ]
message(paste0(round(length(which(palaeozoic$lat_sd > 5))/nrow(palaeozoic), digits = 3)*100, 
               "% of cells have a latitudinal standard deviation value more than 5°"))
cambrian <- df[which(df$time > 485), ]
message(paste0(round(length(which(cambrian$lat_sd > 5))/nrow(cambrian), digits = 3)*100, 
               "% of cells have a latitudinal standard deviation value more than 5°"))
# Ages to plot
